  ...
  try {
    ...
    if (!fontResponse.ok) {
      console.warn(
        styleText(
          "yellow",
          `\nWarning: Failed to download font ${rawFontName} from ${match[1]}: ${fontResponse.status} ${fontResponse.statusText}`,
        ),
      )
      return
    }
    ...
    console.warn(
      styleText(
        "yellow",
        `\nWarning: Network error while fetching font ${rawFontName} weight ${weight}: ${(err as Error).message}`,
      ),
    )
    return
  }
